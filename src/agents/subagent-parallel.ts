...
function buildMergeContent(
  ...
  switch (strategy) {
    ...
    case "llm-summary": {
      ...
      const rawParts = completed.map((r) => JSON.stringify(r.result).slice(0, 500));
      ...
      return {
        ...
        summary: `${completed.length}/${results.length} tasks completed. Summary would be generated by LLM in production.`,
      };
    }
...
export async function executeParallelTasks(
  ...
  async function runWithConcurrencyLimit(task: ParallelTaskDecomposition): Promise<void> {
    while (runningSet.size >= maxConcurrency) {
      await Promise.race(runningSet);
    }
    ...
    const runPromise = executeSingleTask(config.spawnFn, task, config, abortController.signal)
      .then((result) => {
      ...
      .finally(() => {
        runningSet.delete(runPromise);
      });

    runningSet.add(runPromise);
    await runPromise;
  }
