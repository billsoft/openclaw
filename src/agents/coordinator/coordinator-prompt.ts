...
export function buildCoordinatorSystemPrompt(params: {
  ...
  lines.push(
    ...
    "## 2. Your Tools",
    "",
    `- **sessions_spawn** - Spawn a new async worker (preferred for parallelism)`,
  ...
  lines.push(
    "### Worker Results",
    "",
    "Worker results arrive automatically as internal system events. They look exactly like this:",
    "```",
    ...
    "```",
    "",
    "When you receive these events, read the Result. If you are still waiting for other workers, tell the user their status. If ALL expected workers have completed, synthesize a final answer for the user.",
    "",
  );
  ...
  lines.push(
    ...
    "Do NOT use `parallel_spawn` unless you specifically need synchronous blocking execution for trivial queries. Prefer `sessions_spawn` to stay responsive to the user.",
    "",
  );
